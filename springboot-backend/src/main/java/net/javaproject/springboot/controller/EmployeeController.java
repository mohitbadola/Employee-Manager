...
public class EmployeeController {
    ...
    public ResponseEntity<Employee> updateEmployee(@PathVariable long id,@RequestBody Employee employeeDetails){
        Employee updateEmployee =  employeeRepository.findById(id).orElseThrow(()->new ResouceNotFoundException("Employee not exist with id:" +id));
    ...
    @DeleteMapping("{id}")
    public ResponseEntity<HttpStatus>deleteEmplloyee(@PathVariable long id){
        Employee employee = employeeRepository.findById(id).orElseThrow(() -> new ResouceNotFoundException("Employee not exist with id: "+id));
